...
type RegistryShape = Record<string, Record<string, unknown>>

type TagOf<TRegistry extends RegistryShape> = keyof TRegistry & string

type FaultContextValue<TRegistry extends RegistryShape, TTag extends TagOf<TRegistry>> =
  // oxlint-disable-next-line typescript/no-empty-object-type
  {} extends TRegistry[TTag] ? TRegistry[TTag] | undefined : TRegistry[TTag]

type FaultConstructor<TRegistry extends RegistryShape> = new (
  message?: string,
  options?: ErrorOptions
) => FaultBase<TRegistry>

type FaultRegistryOf<TFaultClass extends FaultConstructor<RegistryShape>> =
  TFaultClass extends FaultConstructor<infer R> ? R : never

type FaultTagOf<TFaultClass extends FaultConstructor<RegistryShape>> = TagOf<
  FaultRegistryOf<TFaultClass>
>

type FaultContextOf<
  TFaultClass extends FaultConstructor<RegistryShape>,
  TTag extends FaultTagOf<TFaultClass>,
> = FaultContextValue<FaultRegistryOf<TFaultClass>, TTag>
...
class FaultBase<TRegistry extends RegistryShape> extends Error {
  /**
   ...
   */
  declare readonly __faultierRegistry?: TRegistry
  ...
  get tag(): TagOf<TRegistry> | typeof NO_FAULT_TAG {
    return this.__tag as TagOf<TRegistry> | typeof NO_FAULT_TAG
  }
   ...
   */
  withTag<TTag extends TagOf<TRegistry>>(
    tag: TTag,
    ...args: ContextParam<TRegistry[TTag]>
  ): TaggedFault<this, TTag, FaultContextValue<TRegistry, TTag>> {
    this._tag = tag
    this._context = args[0] as Record<string, unknown> | undefined
    return this as unknown as TaggedFault<this, TTag, FaultContextValue<TRegistry, TTag>>
  }
  ...
  unwrap(): Error[] {
    ...
    while (current) {
      if (IS_FAULT in current) {
        chain.push(current)
        current = (current as unknown as FaultBase<TRegistry>).cause
      } else {
        break
      }
    }
  ...
  getTags(): string[] {
    const chain = this.unwrap()
    return chain.filter((e): e is FaultBase<TRegistry> => IS_FAULT in e).map((fault) => fault.tag)
  }
  ...
  getFullContext(): Record<string, unknown> {
    const chain = this.unwrap()
    const faults = chain.filter((e): e is FaultBase<TRegistry> => IS_FAULT in e)
  ...
  getFullMeta(): Record<string, unknown> {
    const chain = this.unwrap()
    const faults = chain.filter((e): e is FaultBase<TRegistry> => FaultBase.isFault(e))
    const merged: Record<string, unknown> = {}
   ...
   */
  static create<This extends FaultConstructor<RegistryShape>, TTag extends FaultTagOf<This>>(
    this: This,
    tag: TTag,
    ...args: ContextParam<FaultRegistryOf<This>[TTag]>
  ): TaggedFault<InstanceType<This>, TTag, FaultContextOf<This, TTag>> {
    const instance = new this()
    instance._tag = tag
    instance._context = args[0] as Record<string, unknown> | undefined
    return instance as unknown as TaggedFault<InstanceType<This>, TTag, FaultContextOf<This, TTag>>
  }
   ...
   */
  static wrap<This extends FaultConstructor<RegistryShape>>(
    this: This,
    error: unknown
  ): InstanceType<This> {
   ...
   */
  static isFault(value: unknown): value is FaultBase<RegistryShape> {
   ...
   */
  static assert(error: unknown): asserts error is FaultBase<RegistryShape> {
   ...
   */
  static matchTag<
    This extends FaultConstructor<RegistryShape>,
    TTag extends FaultTagOf<This>,
    TResult,
  >(
    this: This,
    error: unknown,
    tag: TTag,
    callback: (
      fault: TaggedFault<FaultBase<FaultRegistryOf<This>>, TTag, FaultContextOf<This, TTag>>
    ) => TResult
    ...
    if (error.tag === tag) {
      return callback(
        error as unknown as TaggedFault<
          FaultBase<FaultRegistryOf<This>>,
          TTag,
          FaultContextOf<This, TTag>
        >
      )
    }
  ...
  static matchTags<
    This extends FaultConstructor<RegistryShape>,
    THandlers extends {
      [K in FaultTagOf<This>]?: (
        fault: TaggedFault<FaultBase<FaultRegistryOf<This>>, K, FaultContextOf<This, K>>
      ) => unknown
    },
  >(
    this: This,
    error: unknown,
    handlers: THandlers
  ): HandlerReturnUnion<THandlers> | typeof UNKNOWN {
    if (!FaultBase.isFault(error)) {
      return UNKNOWN
    }
  ...
  static handle<
    This extends FaultConstructor<RegistryShape>,
    H extends {
      [T in FaultTagOf<This>]: (
        fault: TaggedFault<FaultBase<FaultRegistryOf<This>>, T, FaultContextOf<This, T>>
        // oxlint-disable-next-line typescript/no-explicit-any
      ) => any
    },
  >(this: This, error: unknown, handlers: H): HandlerReturnUnion<H> | typeof UNKNOWN {
    if (!FaultBase.isFault(error)) {
      return UNKNOWN
    }

    const handler = handlers[error.tag as FaultTagOf<This>]
   ...
   */
  static toSerializable<TRegistry extends RegistryShape>(
    fault: FaultBase<TRegistry>
  ): SerializableFault {
   ...
   */
  static fromSerializable<This extends FaultConstructor<RegistryShape>>(
    this: This,
  ...
  static getIssue(fault: FaultLike, options?: Partial<ChainFormattingOptions>): string {
    ...
    return fault
      .unwrap()
      .filter((e): e is FaultBase<RegistryShape> => FaultBase.isFault(e))
...
type FaultClass<TRegistry extends RegistryShape> = FaultConstructor<TRegistry> & typeof FaultBase
 ...
 * ```
 */
export function define<TRegistry extends RegistryShape>(): FaultClass<TRegistry> {
  return class Fault extends FaultBase<TRegistry> {} as FaultClass<TRegistry>
}
